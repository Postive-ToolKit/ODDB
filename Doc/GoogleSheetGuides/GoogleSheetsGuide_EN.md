...
- In Google Sheets, `#ODDB_GROUP`/`#END_GROUP` rows use sky blue, `#TABLE`/`#NAME`/`#TYPE` rows use gray, and `#END_TABLE` rows use soft red; all marker rows use bold white text. Normal per-table tabs also style their `#NAME`/`#TYPE` rows from the first export. Formatting is limited to ODDB-managed columns.
- Google Sheets and CSV imports match fields by the local ODDB field name instead of requiring identical column counts or order. Extra source columns are ignored. A missing local column keeps the current value for an existing row and uses the field default for a new row. Structural ambiguity such as duplicate headers, missing markers, or empty/duplicate row IDs still stops import with a detailed correction message.
