using System;
using System.IO;
using UnityEngine;
using Object = UnityEngine.Object;
#if UNITY_EDITOR
using UnityEditor;
#endif
namespace TeamODD.ODDB.Runtime.Data.Serializers
{
    public class ODDBGenericResourceSerializer<T> : IODDBDataSerializer where T : Object
    {
        public string Serialize(object data)
        {
            #if UNITY_EDITOR
            if (data is not T asset)
                return string.Empty;

            var assetPath = AssetDatabase.GetAssetPath(asset);
            if (string.IsNullOrEmpty(assetPath))
                return string.Empty;
            
            Debug.Log(assetPath);

            var resourcesIndex = assetPath.IndexOf("Resources/", StringComparison.Ordinal);
            if (resourcesIndex < 0)
                return string.Empty;

            var relativePath = assetPath.Substring(resourcesIndex + "Resources/".Length);
            return Path.ChangeExtension(relativePath, null);
            #else
            if (data is Object obj)
                return obj.name;
            return string.Empty;
            #endif
        }

        public object Deserialize(string serializedData)
        {
            if (TryConvertReferenceObject(serializedData, out T result))
                return result;
            return null;
        }
        
        private bool TryConvertReferenceObject(string value, out T result)
        {
            result = null;
            if (string.IsNullOrEmpty(value))
                return false;
            // Load the asset from the path in resources
            result = Resources.Load<T>(value);
            if(result == null) 
                return false;
            return true;
        }
    }
}