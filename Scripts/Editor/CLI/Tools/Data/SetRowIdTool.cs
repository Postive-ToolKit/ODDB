...
namespace TeamODD.ODDB.Editors.CLI.Tools.Data
{
    public class SetRowIdTool : ICliOperation
    {
        ...
        public object Execute(JToken args)
        {
            ...
            if (string.IsNullOrEmpty(tableId) || string.IsNullOrEmpty(rowId) || string.IsNullOrEmpty(newRowId))
                throw new CliOperationException(CliErrorKind.InvalidArg, "tableId, rowId, and newRowId required");
            if (_useCase.GetViewByKey(tableId) is not Table table)
                throw new CliOperationException(CliErrorKind.NotFound, $"table not found: {tableId}");
            if (table.GetRow(rowId) == null)
                throw new CliOperationException(CliErrorKind.NotFound, $"row not found: {rowId}");
            if (!string.Equals(rowId, newRowId, StringComparison.Ordinal) && _useCase.GetRow(newRowId) != null)
                throw new CliOperationException(CliErrorKind.Conflict, $"row id already exists: {newRowId}");
            ...
            {
                throw new CliOperationException(CliErrorKind.Conflict, ex.Message);
            }
