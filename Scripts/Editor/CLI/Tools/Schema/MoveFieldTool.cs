...
{
    public class MoveFieldTool : ICliOperation
    {
        ...
        public object Execute(JToken args)
        {
            ...
            if (string.IsNullOrEmpty(viewId) || args?["oldIndex"] == null || args?["newIndex"] == null)
                throw new CliOperationException(CliErrorKind.InvalidArg, "viewId, oldIndex, newIndex required");
            ...
            if (oldI < 0 || oldI >= n || newI < 0 || newI >= n)
                throw new CliOperationException(CliErrorKind.InvalidArg, "index out of range");
