...
{
    public class SetViewIdTool : ICliOperation
    {
        ...
        public object Execute(JToken args)
        {
            ...
            if (string.IsNullOrEmpty(id) || string.IsNullOrEmpty(newId))
                throw new CliOperationException(CliErrorKind.InvalidArg, "viewId and newViewId required");
            ...
            if (!string.Equals(id, newId, StringComparison.Ordinal) && _useCase.GetViewByKey(newId) != null)
                throw new CliOperationException(CliErrorKind.Conflict, $"view id already exists: {newId}");
