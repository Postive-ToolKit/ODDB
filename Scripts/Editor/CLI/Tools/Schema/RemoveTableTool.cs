...
{
    public class RemoveTableTool : ICliOperation
    {
        ...
        public object Execute(JToken args)
        {
            ...
            if (string.IsNullOrEmpty(id))
                throw new CliOperationException(CliErrorKind.InvalidArg, "tableId required");
            var v = _useCase.GetViewByKey(id);
            if (v == null) throw new CliOperationException(CliErrorKind.NotFound, $"table not found: {id}");
            if (_useCase.GetViewTypeByKey(id) != ODDBViewType.Table)
                throw new CliOperationException(CliErrorKind.InvalidArg, $"{id} is not a Table; use oddb_remove_view");
