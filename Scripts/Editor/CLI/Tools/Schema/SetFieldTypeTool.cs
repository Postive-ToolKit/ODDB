...
{
    public class SetFieldTypeTool : ICliOperation
    {
        ...
        public object Execute(JToken args)
        {
            ...
            if (string.IsNullOrEmpty(viewId) || string.IsNullOrEmpty(typeStr))
                throw new CliOperationException(CliErrorKind.InvalidArg, "viewId and fieldType required");
            if (args?["fieldIndex"] == null)
                throw new CliOperationException(CliErrorKind.InvalidArg, "fieldIndex required");
            int fieldIndex = (int)args["fieldIndex"];

            var view = _useCase.GetViewByKey(viewId);
            if (view == null)
                throw new CliOperationException(CliErrorKind.NotFound, $"view not found: {viewId}");
            if (fieldIndex < 0 || fieldIndex >= view.TotalFields.Count)
                throw new CliOperationException(CliErrorKind.InvalidArg, $"fieldIndex {fieldIndex} out of range (0..{view.TotalFields.Count - 1})");

            var typeKey = typeStr.ToLowerInvariant();
            if (TypeRegistry.GetDescriptor(typeKey) == null)
                throw new CliOperationException(CliErrorKind.InvalidArg, $"unknown fieldType: {typeStr}");
