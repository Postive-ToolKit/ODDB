...
using TeamODD.ODDB.Editors.CLI.Serialization;
using TeamODD.ODDB.Editors.Window;
using TeamODD.ODDB.Runtime;

namespace TeamODD.ODDB.Editors.CLI.Resources
{
    public class TableRowsResource : ICliResource
    {
        ...
        public object Read(string uri)
        {
            ...
            if (view is not Table table)
                throw new CliOperationException(CliErrorKind.NotFound, $"table not found: {id}");

            var rowId = m.Groups["rowId"].Value;
            if (!string.IsNullOrEmpty(rowId))
            {
                var row = table.GetRow(rowId);
                if (row == null) throw new CliOperationException(CliErrorKind.NotFound, $"row not found: {rowId}");
